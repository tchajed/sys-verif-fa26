...
Goose first translates that package in two parts, one _trusted_ (generated by the goose binary) and one _untrusted_ (generated by the proofgen binary). You generally won't interact with the translation process because the output is committed to the repo, but the script `./etc/update-goose.sh` will re-run the translation in case you want to try out some new code.
...
(You might wonder how our proofs end up in the New namespace in Rocq. This is because this is a logical path, and the \_RocqProject is able to map both Perennial and this repo's directories to New.)
...
The heap package we're demoing a proof of depends on std, the verified "Goose standard library".
...
Recall that a Hoare triple `{P} e {Q}` actually means $\forall Φ, P \wand
  (Q \wand Φ) \wand wp(e, Φ)$. `wp_start` introduces the $Φ$, precondition, and wand for the postcondition. If you give it an intro pattern, it uses that to destruct the precondition.
